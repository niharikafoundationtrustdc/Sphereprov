...
const ReservationEntry: React.FC<ReservationEntryProps> = ({ onClose, existingGuests, rooms, onSave, settings }) => {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>, docType: keyof Guest['documents'], isSecondary = false) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => applyDocumentUpdate(docType, reader.result as string, isSecondary);
        ...
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden no-print">
          <div className="w-full lg:w-[380px] border-r bg-slate-50/50 p-4 md:p-8 overflow-y-auto custom-scrollbar space-y-6">
            ...
            <div className="flex gap-2 items-end">
              ...
              <button type="button" onClick={handleSearchGuest} className="bg-orange-500 text-white px-4 py-3 rounded-xl font-black text-[10px] uppercase shadow-lg">Find</button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
               <div className="space-y-1">
                 ...
                 <select className="w-full border-2 p-3 rounded-xl text-[11px] font-black bg-white" value={gender} onChange={e => setGender(e.target.value as any)}>
            ...
            <div className="space-y-1">
              <label className="text-[9px] font-black uppercase text-slate-400 ml-1">Meal Plan</label>
              <select className="w-full border-2 p-3 rounded-xl text-[11px] font-black bg-white" value={mealPlan} onChange={e => setMealPlan(e.target.value)}>
          ...
          <div className="flex-1 p-4 md:p-10 space-y-8 overflow-y-auto custom-scrollbar bg-white">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              <DocBox label="ID Front" src={documents.aadharFront} onUpload={e => handleFileUpload(e, 'aadharFront')} onCapture={() => { setActiveDocCapture({type:'aadharFront', isSecondary:false}); setIsCameraOpen(true); }} />
              <DocBox label="ID Back" src={documents.aadharBack} onUpload={e => handleFileUpload(e, 'aadharBack')} onCapture={() => { setActiveDocCapture({type:'aadharBack', isSecondary:false}); setIsCameraOpen(true); }} />
              <div className="flex flex-col items-center justify-center gap-3 bg-slate-50 border-2 border-dashed border-slate-200 rounded-[2rem] p-4">
                {documents.photo ? <img src={documents.photo} className="w-16 h-16 rounded-full object-cover" /> : <div className="text-[8px] font-black text-slate-300 uppercase">PHOTO</div>}
                <button onClick={() => { setActiveDocCapture({type:'photo', isSecondary:false}); setIsCameraOpen(true); }} className="bg-orange-500 text-white px-4 py-1.5 rounded-lg text-[8px] font-black uppercase">Capture</button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-orange-50/50 p-6 rounded-[2rem] border border-orange-100">
               ...
               <div className="space-y-1">
                 ...
                 <select className="w-full border-2 p-3 rounded-xl text-[11px] font-black bg-white" value={advanceMethod} onChange={e => setAdvanceMethod(e.target.value)}>
                   <option value="Cash">Cash</option>
                   <option value="UPI">UPI</option>
                 </select>
               </div>
               <Inp label="Discount (₹)" type="number" value={discount} onChange={setDiscount} />
            </div>
          </div>

          <div className="w-full lg:w-[320px] border-l bg-slate-50/50 p-6 flex flex-col">
            ...
            <button onClick={handleSave} className="w-full mt-6 bg-orange-600 text-white font-black py-5 rounded-[1.5rem] uppercase shadow-2xl hover:bg-black transition-all text-xs">Authorize Reservation</button>
          </div>
        </div>
      </div>
...
const Inp = ({ label, value, onChange, type = "text" }: any) => (
  <div className="space-y-1 w-full text-left">
    ...
    <input type={type} className="w-full border-2 p-3 rounded-xl text-[11px] font-black text-black bg-white focus:border-orange-500 outline-none" value={value || ''} onChange={e => onChange(e.target.value)} />
  </div>
);

const DocBox = ({ label, src, onUpload, onCapture }: any) => (
  <div className="relative aspect-video bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center overflow-hidden group">
    ...
    <div className="absolute inset-0 opacity-0 group-hover:opacity-100 bg-black/60 flex items-center justify-center gap-1">
       <div className="relative overflow-hidden bg-white p-1 rounded-md cursor-pointer">
       ...
       <button type="button" onClick={onCapture} className="bg-orange-600 text-white p-1 rounded-md text-[7px] font-black uppercase">Snap</button>
    </div>
  </div>
);
